...
Once you created your project, you will see the "Empty game" text. This is normal, as no code has been written yet.
...
The `luau-api` folder was created automatically in your game by vectarine and contains the available modules and their functions.

With a Luau extension / plugin installed, your text editor be able to autocomplete your code using the functions in `luau-api`.

`luau-api` is a great source of documentation and can be thought of as a companion to this manual.
...
Vectarine tries to run at 60 fps, so `time_delta` is at least `0.0166667` seconds but can increase if your rendering code is too slow.
...
function Update(time_delta: number)
    ...
    if io.isKeyJustPressed("Space") then
        ...
        tabs:setActiveTab("another_tab_name", {
            ...
            -- You can also use a custom function as the animationType
            duration = 1.0,
			animationType = "slide_up",
        })
    end

    -- Don't forget to draw tabs widget!
    tabs:draw()
end
```

You can have one main widget per file to split logic and rendering code. There is
more information about using widgets to make complex interfaces in the online guide.
