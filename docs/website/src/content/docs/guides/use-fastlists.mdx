...
export const fastlistExample1 = `local Debug = require("@vectarine/debug")
...
<Code lang="luau" title="scripts/main.luau" code={fastlistExample1} />

You can put `timed` blocks inside one another and inside loops to understand what parts of the code take for most time:

export const fastlistTimedNested = `local Debug = require("@vectarine/debug")
...
<Code lang="luau" code={fastlistTimedNested} />
...
export const fastlistRemoveDuplicatesSlow = `--- Slow version, notice that we have a nested loop, which makes the algorithm O(n^2)
...
<Code lang="luau" code={fastlistRemoveDuplicatesSlow} />

Do the following instead:

export const fastlistRemoveDuplicatesFast = `--- Fast version, we use a set to keep track of seen values, which makes the algorithm O(n)
function removeDuplicates(list)
    local result = {}
    local seen = {}
    for i, v in ipairs(list) do
...
<Code lang="luau" code={fastlistRemoveDuplicatesFast} />
...
export const fastlistObjectsSlow = `-- Let's imagine that something fills this list with objects that have a position and a velocity
local objects = {
    {
        position = Vec.V2(0, 0),
        velocity = Vec.V2(1, 1),
...
<Code lang="luau" code={fastlistObjectsSlow} />

Let's convert this code to use fastlists:

export const fastlistObjectsFast = `local FastList = require("@vectarine/fastlist")
...
<Code lang="luau" code={fastlistObjectsFast} />

export const fastlistVelocityFlipping = `    -- We do something similar for the velocity, but we multiply by (-1, 1) to flip the velocity on the X axis.
    objects.velocity = (velocityWherePositionNegative * Vec.V2(-1, 1)):otherwise(objects.velocity)
end`;

<Code lang="luau" code={fastlistVelocityFlipping} />
...
export const fastlistForLoopSlow = `for i = ... do
    if a.x[i] < b and c.y[i] < d then
        e[i] = modify(e[i])
    end
end`;

<Code lang="luau" code={fastlistForLoopSlow} />

becomes:


export const fastlistForLoopFast = `    local filtered = a:filterBetweenX(a, -math.huge, b):filterBetweenY(c, -math.huge, d)
...
<Code lang="luau" code={fastlistForLoopFast} />
...
export const fastlistDrawImages = `-- Let's assume we have a fastlist of positions named 'positions'.
...
<Code lang="luau" code={fastlistDrawImages} />
...
export const fastlistInteractingIndices = `for i in ... do
    if a[i] < b[i + 1] then
        c[i / 2] = c[i] - c[i - 1]
    end
end`;

<Code lang="luau" code={fastlistInteractingIndices} />
