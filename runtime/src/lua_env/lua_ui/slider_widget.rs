...
impl VectarineWidget for Slider {
    ...
    fn draw(
        ...
        let handle_width = self.handle.0.borrow().size(lua, io_env, &extra).x();
        ...
        let track_height = self.track.0.borrow().size(lua, io_env, &extra).y();
        ...
        let handle_height = self.handle.0.borrow().size(lua, io_env, &extra).y();
